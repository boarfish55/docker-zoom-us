...
RUN apt-get update && \
	...
	apt-get install -qy --no-install-recommends bind9-dnsutils curl \
		ca-certificates google-chrome-stable sudo desktop-file-utils \
		iputils-ping iproute2 lib32z1 libx11-6 libegl1-mesa \
		libxcb-shm0 libglib2.0-0 libgl1-mesa-glx libxrender1 \
		libxcomposite1 libxslt1.1 libgstreamer1.0-0 \
